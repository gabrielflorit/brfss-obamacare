...
**Research question 1:** Given the range of uninsured residents (17% - 5%), is there an association between health care coverage and general health?
...
brfss2013 %>%
	select(Coverage = X_hcvu651, State = X_state, Sleep = X_age80) %>%
	filter(State == 'Alabama') %>%
	na.omit %>%
	group_by(State, Coverage) %>%
	summarise(n = n(), sleep = mean(Sleep)) %>%
  mutate(freq = n / sum(n)) %>%
	head
...
# Convert to data frame for plotting
brfss2013 %>%
	select(Coverage = X_hcvu651, State = X_state, Age = X_age80) %>%
	na.omit %>%
	group_by(State, Coverage) %>%
	summarise(n = n(), Age.median = median(Age)) %>%
  mutate(Uninsured = n / sum(n)) %>%
	filter(Coverage == 'Have health care coverage') %>%
	ggplot(aes(Age.median, Uninsured)) +
	geom_point()
